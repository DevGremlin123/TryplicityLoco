#!/bin/bash
# ============================================================
# TRYPLICITY - 8x NVIDIA H200 SXM — FULL PIPELINE
#
# TWO MODES (so you can save filtered data before training):
#
#   bash run_h200.sh prep     ← Download + AI filter + tokenizer (safe to stop after)
#   bash run_h200.sh train    ← Train the 9.4B model on filtered data
#   bash run_h200.sh          ← Run everything back-to-back
#
# ONLY 2 COMMANDS TOTAL:
#   1. scp -r ./Tryplicity <pod>:~/Tryplicity
#   2. cd ~/Tryplicity && bash run_h200.sh prep
#      (check your filtered data, then...)
#      bash run_h200.sh train
#
# HARDWARE: 8x H200 SXM (141 GB HBM3e each, 1,128 GB total)
# STRATEGY: FSDP (Fully Sharded Data Parallel) — required because
#           DDP needs 150 GB/GPU but H200 only has 141 GB
# COST:     ~$5.26 for 11 min at $28.72/hr (8x $3.59/GPU)
#
# ============================================================
...
if n >= 8:
    ...
    if per_gpu < 150:
        print(f'  NOTE: {per_gpu:.0f} GB/GPU — using FSDP (shards model across GPUs)')
    else:
        print(f'  NOTE: {per_gpu:.0f} GB/GPU — DDP would also work')
"
    echo ""
...
# ================================================================
# TRAIN MODE: FSDP training on 8x H200 SXM
# ================================================================
run_train() {
    echo ""
    echo "  ╔══════════════════════════════════════════════════════════╗"
    echo "  ║       TRYPLICITY — TRAINING 9.4B MODEL (FSDP)          ║"
    echo "  ║       8x NVIDIA H200 SXM — 141 GB HBM3e each          ║"
    echo "  ╚══════════════════════════════════════════════════════════╝"
    echo ""
    ...
    if [ ! -d "data/filtered" ] || [ -z "$(ls data/filtered/*.jsonl 2>/dev/null)" ]; then
        ...
        echo "  Run 'bash run_h200.sh prep' first."
        exit 1
    fi
    ...
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    echo "  LAUNCHING: 9.4B model on ${NUM_GPUS}x H200 SXM"
    echo "  Strategy: FSDP FULL_SHARD (ZeRO Stage 3)"
    echo "  WHY FSDP: H200 has 141 GB, DDP needs 150 GB = OOM"
    echo "  FSDP:     ~41 GB / 141 GB per GPU (100 GB headroom)"
    ...
    echo "  ║  scp <pod>:~/Tryplicity/checkpoints/final_h200.pt ./ ║"
    echo "  ╚══════════════════════════════════════════════════════════╝"
}
...
case "$MODE" in
    ...
    all)
        ...
        ;;
    *)
        echo "Usage: bash run_h200.sh [prep|train|all]"
        echo ""
        echo "  prep   — Download data, AI quality filter, build tokenizer"
        echo "  train  — Train the 9.4B model with FSDP (requires prep first)"
